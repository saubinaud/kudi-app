    ...
    <div className="max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-1 bg-stone-100 rounded-lg p-0.5">
            <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-md text-xs transition-colors duration-100 ${viewMode === 'list' ? 'bg-white shadow text-stone-900' : 'text-stone-500'}`}>
              Ventas
            </button>
            <button onClick={() => setViewMode('pos')} className={`px-3 py-1.5 rounded-md text-xs transition-colors duration-100 ${viewMode === 'pos' ? 'bg-white shadow text-stone-900' : 'text-stone-500'}`}>
              POS
            </button>
          </div>
      ...
      {viewMode === 'pos' ? (
        <div className="flex flex-col lg:flex-row gap-4 pb-20 lg:pb-0">
          ...
          <div className="flex-1">
            ...
            <div className="mb-4">
              <input type="text" value={posSearch} onChange={e => setPosSearch(e.target.value)}
                className={cx.input + ' text-sm'} placeholder="Buscar producto..." />
            </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
              {posFilteredProducts.map(p => (
                <button key={p.id} onClick={() => addToCart(p)}
                  ...
                  {p.imagen_url ? (
                    <img src={p.imagen_url} className="w-full aspect-square object-cover rounded-lg mb-1.5" />
                  ) : (
                ...
                <div className="space-y-2 mb-4">
                  {cartItems.map((item, i) => (
                    <div key={i} className="flex items-center gap-2 bg-stone-50 rounded-lg p-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <input type="number" min="0" step="0.01"
                            ...
                            className="w-14 text-[10px] border rounded px-1 py-0.5" placeholder="Desc" />
                        </div>
                      </div>
                      ...
                      <button onClick={() => removeFromCart(i)} className="text-stone-400 hover:text-rose-500">
                        <X size={14} />
                      </button>
                    </div>
                  ))}
                  ...
                  {showPosCheckout ? (
                    <div className="border-t border-stone-200 pt-3 space-y-3">
                      ...
                      <div className="flex gap-2">
                        <select value={posCliente.tipo_doc} onChange={e => setPosCliente(p => ({...p, tipo_doc: e.target.value}))}
                          className={cx.input + ' w-24 text-sm'}>
                          <option value="DNI">DNI</option>
                          <option value="RUC">RUC</option>
                        </select>
                        <div className="relative flex-1">
                          <input type="text" value={posCliente.num_doc}
                            onChange={e => {
                            ...
                            className={cx.input + ' text-sm'} placeholder={posCliente.tipo_doc === 'DNI' ? '12345678' : '20123456789'}
                            maxLength={posCliente.tipo_doc === 'DNI' ? 8 : 11} />
                          {buscandoDoc && <div className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 border-2 border-stone-300 border-t-stone-600 rounded-full animate-spin" />}
                        </div>
                      </div>

                      {/* Name (auto-filled or manual) */}
                      <input type="text" value={posCliente.nombre}
                        onChange={e => setPosCliente(p => ({...p, nombre: e.target.value}))}
                        className={cx.input + ' text-sm'} placeholder="Nombre del cliente" />

                      {/* Email + Phone row */}
                      <div className="grid grid-cols-2 gap-2">
                        <input type="email" value={posCliente.email}
                          onChange={e => setPosCliente(p => ({...p, email: e.target.value}))}
                          className={cx.input + ' text-sm'} placeholder="Email (opcional)" />
                        <input type="tel" value={posCliente.telefono}
                          onChange={e => setPosCliente(p => ({...p, telefono: e.target.value}))}
                          className={cx.input + ' text-sm'} placeholder="Teléfono" />
                      </div>
                      ...
                      <div className="border-t border-stone-200 pt-3">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => setShowPosCheckout(false)} className={cx.btnGhost + ' flex-1 text-sm'}>Volver</button>
                          <button onClick={handlePOSCheckout} disabled={saving}
                    ...
                    <>
                      <div className="border-t border-stone-200 pt-3 space-y-1">
                        ...
                        <div className="flex justify-between text-lg font-bold"><span>Total</span><span className="text-[var(--accent)]">{formatCurrency(cartTotal)}</span></div>
                      </div>
                      <button onClick={() => setShowPosCheckout(true)}
          ...
          {/* Mobile: Cart below product grid (visible on small screens only) */}
          <div className="lg:hidden">
            <div className={cx.card + ' p-4'}>
              ...
              {cartItems.length > 0 && showPosCheckout && (
                <div className="border-t border-stone-200 pt-3 space-y-3">
                  <h3 className="font-bold text-stone-900">Datos del cliente</h3>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xs max-w-[95vw] p-5">
            ...
            <div className="space-y-1 max-h-60 overflow-y-auto">
              {(variantModal.variantes || []).map(v => (
                <button key={v.id} onClick={() => addVariantToCart(variantModal, v)}
                  className="w-full text-left p-2.5 rounded-lg hover:bg-stone-50 flex justify-between items-center">
      ...
      {/* Mobile bottom bar for POS cart */}
      {viewMode === 'pos' && cartItems.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 p-3 flex items-center justify-between lg:hidden z-40">
          ...
          <button onClick={() => setShowPosCheckout(true)} disabled={saving}
      ...
      {/* POS post-checkout success overlay */}
      {lastSaleId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
